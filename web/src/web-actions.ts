...
async function setCheckboxState(locator: Locator, checked: boolean, timeoutMs: number): Promise<void> {
  ...
  try {
    ...
    await input.evaluate((el: HTMLInputElement, wantChecked: boolean) => {
...
export async function expectSelected(locator: Locator, value: string, timeoutMs = 30_000): Promise<void> {
  await expect(locator).toHaveValue(value, { timeout: timeoutMs });
}

export async function waitForVisible(locator: Locator, timeoutMs = 30_000): Promise<void> {
  await expect(locator.first()).toBeVisible({ timeout: timeoutMs });
}

export async function waitForHidden(locator: Locator, timeoutMs = 30_000): Promise<void> {
  await expect(locator).not.toBeVisible({ timeout: timeoutMs });
}

export async function goBack(page: Page, timeoutMs = 30_000): Promise<void> {
  await page.goBack({ timeout: timeoutMs });
}

export async function navigateTo(page: Page, url: string, timeoutMs = 30_000): Promise<void> {
  await page.goto(url, { timeout: timeoutMs, waitUntil: "domcontentloaded" });
}

export async function takeScreenshot(page: Page, filePath?: string) {
